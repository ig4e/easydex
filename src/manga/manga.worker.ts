    ...
    for (let i = 0; i < 100; i++) {
      const { data: pageData } = await axios({
        url: `https://api.mangadex.org/manga?limit=${LIMIT}&offset=${
          i * LIMIT
        }&includes[]=cover_art&includes[]=author&includes[]=artist&contentRating[]=safe&contentRating[]=suggestive&contentRating[]=erotica&order[rating]=desc`,
      });
          ...
          if (await this.prisma.manga.count({ where: { dexId: manga.id } })) {
            ...
            const mangaDB = await this.prisma.manga.create({
              data: {
                ...
                originalLanguage:
                  manga.attributes?.originalLanguage?.toUpperCase(),
            ...
            await this.prisma.mangaCover.createMany({
              data: covers.data.map((cover) => ({
                ...
                locale: cover.attributes.locale.toUpperCase(),
                version: Number(cover.attributes.version),
                volume: Number(cover.attributes.volume),
